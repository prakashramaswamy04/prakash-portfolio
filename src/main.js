...
document.querySelector('#journey-track').innerHTML = journey.map(([item,type],i)=>`<div class="journey-step ${type}${i > 3 ? ' journey-private' : ''}"${i > 3 ? ' aria-label="Additional technical journey details"' : ''}><span>${String(i+1).padStart(2,'0')}</span><i></i><strong>${item}</strong></div>`).join('');
